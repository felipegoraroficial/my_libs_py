...
def test_quoted_newline_and_balance_stats():
    assert has_quoted_newline_from_text('"first\nsecond"')
    assert has_quoted_newline_from_text('"first\rsecond"')
    assert not has_quoted_newline_from_text('"first"\nsecond')
    assert not has_quoted_newline_from_text('"a""b"')

    stats = line_quote_balance_stats('"open\nclosed"\n"bad', '"')
    assert stats == {
        "lines_checked": 3,
        "unbalanced_count": 3,
        "unbalanced_examples_idx": [0, 1, 2],
    }
    stats = line_quote_balance_stats("x\n" * 501, '"')
    assert stats["lines_checked"] == 500
