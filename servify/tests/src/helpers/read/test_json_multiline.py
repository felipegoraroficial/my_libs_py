...
def test_json_multiline_detection(tmp_path, monkeypatch, log):
    import importlib

    json_module = importlib.import_module(
        "servify.src.helpers.read.json.detectar_json_multiline"
    )
    ...
    path.write_text('{\n"a": 1\n}\n', encoding="utf-8")
    assert detectar_json_multiline(str(path), log=log) is True
    path.write_text("1\n2\n", encoding="utf-8")
    assert detectar_json_multiline(str(path), log=log) is False
    path.write_text("[\n1\n]\n", encoding="utf-8")
    assert detectar_json_multiline(str(path), log=log) is True
