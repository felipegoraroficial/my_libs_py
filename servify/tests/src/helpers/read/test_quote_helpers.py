...
def test_quote_helpers_cover_quote_and_escape_variants():
    assert detect_dominant_quote_char('"a", "b", "c"') == '"'
    assert detect_dominant_quote_char("'a', 'b', 'c'") == "'"
    assert detect_dominant_quote_char("\"a\" and 'b'") is None
    assert detect_escape_style('"a""b"') == "csv_double"
    assert detect_escape_style('"a\\"b"') == "backslash"
    assert detect_escape_style("'a''b'", "'") == "csv_double"
    assert detect_escape_style("'a\\'b'", "'") == "backslash"
    assert detect_escape_style("plain") == "none"
